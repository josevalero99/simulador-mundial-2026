...
/** Logistic scale for the strength->probability curve. Lower => sharper. */
const SCALE = 12
/** Base expected goals per side before strength adjustment. */
const BASE_MEAN = 1.3
/** How strongly the strength delta nudges expected goals. */
const STRENGTH_SWING = 0.9
/** Cap on the per-match goals sampled per side (keeps scorelines sane). */
const MAX_GOALS = 7

/**
 * Probability the home team beats the away team based on relative FIFA rank.
 * A much lower (= stronger) home rank pushes this toward 1.
 */
export function pHome(
  ...
  const rankHome = fifaRank(homeId)
  const rankAway = fifaRank(awayId)
  return 1 / (1 + 10 ** ((rankHome - rankAway) / SCALE))
}
...
/**
 * Produces a probabilistic scoreline from relative strength. The stronger side
 * (lower fifaRank) gets a higher expected-goals mean. Group-stage results may be
 * level (a draw); knockout callers must resolve ties themselves (see pickWinner).
 */
export function expectedResult(
  ...
  const ph = pHome(homeId, awayId, fifaRank)
  // Map win-prob (0..1) onto a symmetric strength swing around BASE_MEAN.
  // ph = 0.5 -> no swing; ph -> 1 boosts home mean and suppresses away mean.
  const swing = (ph - 0.5) * 2 * STRENGTH_SWING
  const homeMean = Math.max(0.15, BASE_MEAN + swing)
  const awayMean = Math.max(0.15, BASE_MEAN - swing)
  return {
    homeGoals: samplePoisson(homeMean, rng),
    awayGoals: samplePoisson(awayMean, rng),
  }
}
 ...
 * simulated via `rng`.
 */
export function simulateOnce(rng: Rng, base?: Match[]): SimResult {
  ...
  // 2. Fill remaining null group matches via expectedResult.
  for (const m of matches) {
    if (m.homeGoals === null || m.awayGoals === null) {
...
export function runMonteCarlo(
  ...
  for (let i = 0; i < n; i++) {
    const { reached } = simulateOnce(rng, base)
